...
export class HomePage {
  ...
  constructor(public navCtrl: NavController, public geolocation: Geolocation, public platform:Platform,private socialSharing: SocialSharing) {}
  ...
  private loadMap(lat, lng) {
    ...
    share(){
      this.geolocation.getCurrentPosition().then(result => {
        // this.loadMap(result.coords.latitude, result.coords.longitude);
        this.socialSharing.shareViaWhatsAppToReceiver('6192531306', `estou em ${result.coords.latitude}, ${result.coords.longitude}`);
      });
